...
func printScreen() {
	...
	fmt.Println("1. See Employee History")
	fmt.Println("2. Add Contract (Simple)")
	fmt.Println("3. Read Contract (Simple)")
	fmt.Println("4. Get All Contracts")
	fmt.Println("5. Issue Dispute")
	fmt.Println("6. Read Contract (Unorganized)")
	fmt.Println("7. Close Dispute")
	fmt.Println("8. Respond To A Dispute")
	fmt.Println("9. Terminate Contract")
	fmt.Println("10. Create A Contract")
	fmt.Println("11. Approve Contract")
	fmt.Println("12. Extend Contract")
	fmt.Println("13. Employer History")
...
func extendContract() {
	...
	fmt.Print("Enter the extension date in the following format: 01/01/2023. Do note that you must add the 0 in 01:  ")
...
// Get will return a token without any spaces. I.e you have to add white space after "Bearer" --> "Bearer "
// 100% tested
...
"args": ["{'ID': '` + input + `','Status': 'Active','Notes': 'N/A','Start date': '01/01/2022','End date': '08/08/2023','Extension details': 'N/A','Employer': {        'ID': 'Comp-1',        'Name': 'CompanyA',       'Employer address and contact details': 'Firstst,Riyadh1234','Country': 'SaudiArabia'      },'Employee': {        'ID': '441101772',        'Name': 'JohnDoe',        'Employee address and contact details': 'Secondst,NewDelhi3342',        'Country': 'India'      },'Job': {        'Position': 'Developer',        'Level': 'Senior',        'Description': 'Manageteamsofjuniordevelopers'      },'Benefits': {        'Currency': 'SAR',        'Salary': 10000,        'Annual increase': '3-7',        'Annual leave': '30days','Housing': 2000,        'Allowances': 1500,'Other benefits': 'Schoolingforchildrenandyearlytickets'      }, 'Disputes': [        {          'ID': 'D123',          'Status': 'Closed',          'Last updated date': '01/10/2023',          'Content': 'Emploerdidnotprovidethetravelticketsformyannualleavein2022','Response': [            {              'ID': 'Res123',              'Last updated date': '01/12/2023',              'Content': 'Theemployeewascompensated'            }          ]}      ]    }"  ]}`) //
...
func createContract() {
	...
	fmt.Println("Hello, Creating a contract from scratch involes alot of vairables.")
	fmt.Println("This is why I need you to go to the file contract.json. ")
	...
	input, err := ioutil.ReadFile("contract.json")
	if err != nil {
		fmt.Println("Error")
		panic(err)
